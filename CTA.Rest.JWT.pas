...
{$SCOPEDENUMS ON}
  ...
  TBase64URLEncoder = class(TIdEncoder3to4)
  protected
    procedure InitComponent; override;
  end;

  TBase64URLDecoder = class(TIdDecoder4to3)
  protected
    procedure InitComponent; override;
  end;
...
begin
  ...
  Result := System.DateUtils.DateTimeToUnix(AValue);
end;

function UnixToDateTime(AValue: Int64; AReturnUTC: Boolean): TDateTime;
begin
  Result := System.DateUtils.UnixToDateTime(AValue);
...
begin
  ...
  lHMAC := TIdHMACSHA512.Create;
  try
    lHMAC.Key := ToBytes(Key, TEncoding.UTF8);
    Result := TBytes(lHMAC.HashValue(ToBytes(Input, TEncoding.UTF8)));
  finally
    lHMAC.Free;
  end;
end;

function URLSafeB64encode(ABytes: TBytes): string; overload;
var
  lEncoder: TBase64URLEncoder;
begin
  lEncoder := TBase64URLEncoder.Create(nil);
  try
    Result := StringReplace(lEncoder.EncodeBytes(ABytes), '=', '', [rfReplaceAll]);
  finally
    lEncoder.Free;
  end;
end;

function URLSafeB64encode(AText: String): string; overload;
begin
  Result := URLSafeB64encode(TBytes(ToBytes(AText, TEncoding.UTF8)));
end;

function URLSafeB64decode(AText: String): string; overload;
var
  lDecoder: TBase64URLDecoder;
begin
  case Length(AText) mod 4 of
    0:
      ;
    2:
      AText := AText + '==';
  ...
  lDecoder := TBase64URLDecoder.Create(nil);
  try
    Result := lDecoder.DecodeString(AText, TEncoding.UTF8);
  finally
    lDecoder.Free;
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      lHeaderEncoded := URLSafeB64encode(lHeader.ToString);
      lPayloadEncoded := URLSafeB64encode(lPayload.ToString);
      lToken := lHeaderEncoded + '.' + lPayloadEncoded;
      lBytes := HMAC(HMACAlgorithm, lToken, FSecretKey);
      lHash := URLSafeB64encode(lBytes);
      Result := lToken + '.' + lHash;
    finally
      lPayload.Free;
    end;
...
var
  lPieces: TStringDynArray;
  lAlgName: string;
begin
  Result := False;
  Error := '';
  lPieces := SplitString(Token, '.');
  ...
  Header := TJDOJsonBaseObject.Parse(URLSafeB64decode(lPieces[0])) as TJDOJSONObject;
  try
    ...
    Payload := TJDOJsonBaseObject.Parse(URLSafeB64decode(lPieces[1])) as TJDOJSONObject;
    try
      ...
      Result := Token = lPieces[0] + '.' + lPieces[1] + '.' +
        URLSafeB64encode(HMAC(lAlgName, lPieces[0] + '.' + lPieces[1], FSecretKey));
...
begin
  FCustomClaims.Items['lvw'] := IntToStr(Value);
end;

{ TBase64URLEncoder }

procedure TBase64URLEncoder.InitComponent;
begin
  inherited;
  FFillChar := '=';
  FCodingTable := 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
end;

{ TBase64URLDecoder }

procedure TBase64URLDecoder.InitComponent;
begin
  inherited;
  FFillChar := '=';
  FCodingTable := 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
  ConstructDecodeTable(FCodingTable, FDecodeTable);
end;
